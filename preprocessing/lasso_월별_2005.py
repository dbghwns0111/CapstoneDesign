...
# 1. CSV 파일 불러오기
df = pd.read_csv('data/fred_data_2005_2023.csv')
...
# 10. 계수 저장 (CSV)
coef_df.to_csv('data/lassoCV_2005_2023.csv', index=False)
...
# 알파 값이 0.003로 설정된 경우의 계수 출력
lasso = Lasso(alpha=0.003, max_iter=5000)
lasso.fit(X_train, y_train)
y_pred = lasso.predict(X_test)
mse = mean_squared_error(y_test, y_pred)
r2 = r2_score(y_test, y_pred)
print(f"✅ MSE (alpha=0.003): {mse:.4f}")
print(f"✅ R² Score (alpha=0.003): {r2:.4f}")
print(f"🔢 비제로 계수 개수 (alpha=0.003): {(lasso.coef_ != 0).sum()}/{len(lasso.coef_)}")
# 계수 출력 및 정렬
coef_df_0_005 = pd.DataFrame({
    'Feature': X.columns,
    'Coefficient': lasso.coef_
}).sort_values(by='Coefficient', key=abs, ascending=False)
print("\n📊 주요 변수 계수 (alpha=0.003):")
print(coef_df_0_005)